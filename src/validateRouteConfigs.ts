...
const getScreenComponent = (routeConfig: any) =>
  !routeConfig ? null : routeConfig.screen ? routeConfig.screen : routeConfig

export const validateRouteConfigs = (routeConfigs: NavigationRouteConfigMap) => {
  const routeNames = Object.keys(routeConfigs)
  if (routeNames.length !== 2) {
    throw new Error('Please specify two routes when configuring a master detail navigator.')
  }

  const checkForRoute = (routeName: string) => {
